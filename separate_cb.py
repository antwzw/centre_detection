...
# 需要识别的图片的路径
INPUT_PATH = '/Users/zhengbaoqin/Desktop/bigcircle/1109/大圆_3cm'
...
def findAllSquares(boolImg: np.ndarray):
    ...
    pointMin = round(width * height * 0.00002)
...
def drawTable(squares: np.ndarray):
    ...
    for i in range(0, 100):
        cb.append([-1] * 100)
    findNeighbours(cb, 0, [50, 50], visited)
...
def separateChessboard2(filePath=None):
    ...
    # left = getPartChessboard2(lines[[7, 1, 4, 3]], gray)
    # right = getPartChessboard2(lines[[7, 1, 2, 5]], gray)
    left = getPartChessboard2(lines[[0, 1, 4, 3]], gray)
    right = getPartChessboard2(lines[[0, 1, 2, 5]], gray)
    # imgs = np.empty((3, len(up), len(up[0])), np.uint8)
    imgs = np.empty((3, len(left), len(left[0])), np.uint8)
    # imgs[0] = up
    imgs[1] = left
    imgs[2] = right
...
def getFourCorners(filePath=None):
    ...
    try:
        ...
        # print(len(upCorners))
        ret, leftCorners = cv2.findChessboardCorners(left, (CHESSBOARD_SIZE[2] - 1, CHESSBOARD_SIZE[1] - 1), None)
        ...
        ret, rightCorners = cv2.findChessboardCorners(right, (CHESSBOARD_SIZE[3] - 1, CHESSBOARD_SIZE[1] - 1), None)
    ...
    result[0] = getCorner2(leftCorners, 'upleft')
    result[1] = getCorner2(rightCorners, 'upright')
