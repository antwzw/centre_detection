...
import os
import separate_cb
from big_circle import identify_center
separate_cb.CHESSBOARD_SIZE = CHESSBOARD_SIZE
from separate_cb import getFourCorners

graphtype=38       #参考小圆柱中心（范例0cm设定为38 (y=3)，范例3cm设定为16 (y=1)  x+11*y）
bolt_num=8
separate_cb.CHESSBOARD_SIZE = CHESSBOARD_SIZE
address=r'C:\Users\admin\Desktop\Codes-Zhengwei-20230528'
append1='\*.jpg'
append2='\{num}.jpg'

Canny=r'C:\Users\admin\Desktop\Codes-Zhengwei-20230528\Canny\{num}.jpg'
RESULTcrop=r'C:\Users\admin\Desktop\Codes-Zhengwei-20230528\crop\{num}.jpg'
Hed=r'C:\Users\admin\Desktop\Codes-Zhengwei-20230528\hed\{num}.jpg'

SOURCE_CALIBRATION = r'C:\Users\admin\Desktop\Codes-Zhengwei-20230528\0517\c\*.jpg'
SOURCE = r'C:\Users\admin\Desktop\Codes-Zhengwei-20230528\0517\s\*.jpg'

JiBian=r'C:\Users\admin\Desktop\Codes-Zhengwei-20230528\jibian\{num}.jpg'
RESULT =r'C:\Users\admin\Desktop\Codes-Zhengwei-20230528\part1\{num}.jpg'
SOURCE_PATH =r'C:\Users\admin\Desktop\Codes-Zhengwei-20230528\part1\*.jpg'
RESULT_PATH =r'C:\Users\admin\Desktop\Codes-Zhengwei-20230528\part2\{num}.jpg'
SOURCE_PATHnew =r'C:\Users\admin\Desktop\Codes-Zhengwei-20230528\part2\*.jpg'
PROTO_PATH =r'C:\Users\admin\Desktop\Codes-Zhengwei-20230528\deploy.prototxt'
MODEL_PATH =r'C:\Users\admin\Desktop\Codes-Zhengwei-20230528\hed_pretrained_bsds.caffemodel'

CAMERA_PARA_PATH = r'C:\Users\admin\Desktop\Codes-Zhengwei-20230615\serverV2\0517\data.xlsx'
# 输入图片位置
IMG_PATH = r'C:\Users\admin\Desktop\Codes-Zhengwei-20230615\serverV2\0517\b'    #大棋盘格图片
IMG_NUM = 8# 照片数量
IMG_NUM2=16
WIDTH = 18# 棋盘格的宽度
HEIGHT = 18# 棋盘格的高度
...
def local_grid(gray, param):
    ...
    circleshed = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 1, 30,
    ...
    '''km = KMeans(n_clusters=6).fit(circleshed[0, :, 0:2])
    # km = circleshed[0,:,0:2]
    ...
    mins = df.sort_values('Distance', ascending=False).tail(bolt_num)
    # print(mins)
    
    for index, i in mins.iterrows():
         center = (i['X'].astype("int"), i['Y'].astype("int"))
         cv2.circle(canny, center, 1, (255, 255, 255), 3)
         radius = i['Radius'].astype("int")
         cv2.circle(canny, center, radius, (255, 255, 255), 3)
    #cv2.imwrite(Canny.format(num = count+1), canny)
    ...
    for index, i in mins.iterrows():
        ...
        y_sum = y_sum + i['Y']
    x_sum = x_sum + 1
    #print('measured center: (', x_sum / 6, ",", y_sum / 6,")")

    return result,x_sum,y_sum

def detect_bolts(img, net,ii):
    size = img.shape
    ...
    gray2 = HED(img,net)
    #这里加了两行
    #cv2.imshow("hed", gray2)
    #cv2.waitKey(0)
    #cv2.imwrite(Hed.format(num = ii), gray2)
    
    
    result,x_sum,y_sum = local_grid(gray2,18)##
    x_diff = x_sum / bolt_num - center_x
    y_diff = y_sum / bolt_num - center_y
    aa=(x_sum/bolt_num-center_xx)/15
    bb=(y_sum/bolt_num-center_yy)/15
...
if __name__ == '__main__':
    ...
    for image in images:
        ...
        # TODO
        cv2.imwrite(JiBian.format(num = i), undistorted)
        #
    ...
    for count, image in enumerate(images):
        ...
        #这里加了两行
        #cv2.imshow("hed", cropped)
        #cv2.waitKey(0)
        i=i+1
        cv2.imwrite(RESULTcrop.format(num=i), cropped)
        # result = detect_bolts(image,net)
        ii=ii+1
        a,b,aa,bb,result,diff = detect_bolts(cropped, net,ii)
        if(aa>0 and bb>0):
            if diff>30:
                continue
            print('误差：',diff)
    ...
    print('计算获得的两中心偏移量(mm):', diff)
    print('Finished.')'''
    a,b= identify_center(CAMERA_PARA_PATH, IMG_PATH, 15, 35, 35, True, (35, 35, 5, 5, 5))
    diffa=a-aam
    diffb=b-bbm
    print('大圆计算中心坐标(单位网格)(平均):[',a,',',b,']')
    x=graphtype%11
